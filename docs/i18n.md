...
This site utilizes a simple custom hook to provide i18n support. The hook is called `useI18n` and is located in [hooks/useI18n.ts](). The hook relies on a translation file provided by the top-level ancestor based on language code. Each translation file is a JSON with keys such as `"audio.open_transcript_modal"` which correspond to the translation of that particular word or phrase. The hook itself relies on a `LanguageContext` (see [hooks/useLanguageContext.ts]()), which is a simple `Context` that is used at the top level of each translated route:
